...
def treeNet(requests, times):
    ...
    # CLI(net)
    print(s1.cmd('ovs-ofctl add-flow s1 priority=500,in_port=1,actions=output:2'))
    print(s1.cmd('ovs-ofctl add-flow s1 priority=500,in_port=2,actions=output:1'))
    print(s2.cmd('ovs-ofctl add-flow s2 priority=500,in_port=1,actions=output:2'))
    print(s2.cmd('ovs-ofctl add-flow s2 priority=500,in_port=2,actions=output:1'))
    print(s3.cmd('ovs-ofctl add-flow s3 priority=500,in_port=1,actions=output:2'))
    print(s3.cmd('ovs-ofctl add-flow s3 priority=500,in_port=2,actions=output:1'))
